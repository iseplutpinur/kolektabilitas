...
class Kolektibilitas extends Render_Controller
{

    public function index()
    {
        // Page Settings
        $this->title = 'Kolektibilitas';
        $this->navigation = ['Kolektibilitas'];
        $this->plugins = ['datatables'];

        // Breadcrumb setting
        $this->breadcrumb_1 = 'Dashboard';
        $this->breadcrumb_1_url = base_url() . 'admin/dashboard';
        $this->breadcrumb_3 = 'Kolektibilitas';
        $this->breadcrumb_3_url = '#';
        // content
        $this->content      = 'kolektibilitas';
    ...
    public function export_pdf()
    {
        ...
        foreach ($details as $i => $detail) {
            ...
            $body_table .= "<td>{$detail->dari}</td>";
            $body_table .= "<td>{$detail->sampai}</td>";
    ...
    public function form_import_excel()
    {
        ...
        $col_start = "A";
        $col_end = "F";
        $title_excel = "Form Import Data Kolektibilitas";
        // Header excel ================================================================================================
        $spreadsheet = new PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $row += 1;
        $sheet->mergeCells($col_start . $row . ":" . $col_end . $row)
            ->setCellValue("A$row", "Form Import Data Kolektibilitas");
        $sheet->getStyle($col_start . $row . ":" . $col_end . $row)->applyFromArray([
            "font" => [
                "bold" => true,
                "size" => 13
    ...
    public function import_excel()
    {
        ...
        foreach ($array_from_excel as $data) {
            if ($start > 5) {
                ...
                $sampai = $data[3];
                $keterangan = $data[4];
                $status = $data[5] == 'Digunakan' || $data[6] == 1 ? 1 : 0;

                $exe = $this->model->insert($nama, $keterangan,  $dari, $sampai, $status, $id);
                if (!$exe) {
                    $result = false;
                }
            }
    ...
    function __construct()
    {
        ...
        $this->load->model("KolektibilitasModel", 'model');
